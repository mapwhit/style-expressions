...
class StyleExpression {
  ...
  evaluateWithoutErrorHandling(globals, feature, featureState) {
    if (this.#globalState) {
      globals.globalState = this.#globalState;
    }
...
class ZoomDependentExpression {
  #globalState;
  constructor(kind, expression, zoomCurve, globalState) {
    this.kind = kind;
...
function createPropertyExpression(expression, propertySpec, globalState) {
  expression = createExpression(expression, propertySpec, globalState);
  ...
  return success(
    isFeatureConstant
      ? new ZoomDependentExpression('camera', expression.value, zoomCurve, globalState)
      : new ZoomDependentExpression('composite', expression.value, zoomCurve, globalState)
  );
}
